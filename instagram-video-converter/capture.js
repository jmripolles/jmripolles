// capture.js — Render the React animation frame-by-frame and encode to MP4.
// Usage: node capture.js [output.mp4]
//
// Requires: ffmpeg in PATH, Playwright Chromium at /opt/pw-browsers/
...
const WIDTH = 1080;
const HEIGHT = 1920;
const OUTPUT = process.argv[2] || path.join(PROJECT_DIR, 'reel-ies-maestrat.mp4');
...
async function main() {
  ...
  // 2. ffmpeg — reads PNG frames from stdin
  const ffmpegArgs = [
    '-y',
  ...
  console.log(`[capture] ffmpeg ${ffmpegArgs.join(' ')}`);
  ...
  for (let i = 0; i < TOTAL_FRAMES; i++) {
    ...
    // Advance animation time (flushSync inside — DOM is updated before returning)
    await page.evaluate((time) => window.__setVideoTime(time), t);

    // One extra rAF to let any CSS transitions/paints settle
