  ...
  return (
    <Sprite start={54} end={62}>
      <div style={{
        ...
        <Sprite start={55.6} end={61.9}>
          <VAdmissionCallout x={540} y={1000} />
        </Sprite>

        {/* Stacked contact blocks */}
        <Sprite start={55.8} end={61.9}>
          <VContactBlock x={80} y={1200} label="CONTACTE" lines={[
            "Camí Font de Morella, 2",
            "12.170 · Sant Mateu (Castelló)",
          ]} accent={VPAL.teal} />
        </Sprite>
        <Sprite start={56.1} end={61.9}>
          <VContactBlock x={80} y={1380} label="TELÈFON · CORREU" lines={[
            "964 33 60 90",
            "12004400@edu.gva.es",
          ]} accent={VPAL.green} />
        </Sprite>
        <Sprite start={56.4} end={61.9}>
          <VContactBlock x={80} y={1560} label="WEB · XARXES" lines={[
...
function VAdmissionCallout({ x, y }) {
  const time = useTime();
  const { localTime } = useSprite();
  const entry = clamp(localTime / 0.5, 0, 1);
  // Blink: smooth pulse between 0.55 and 1.0 every ~0.9s
  ...
  return (
    <div style={{
      ...
      opacity: entry,
      width: 920,
      background: VPAL.warm,
      borderRadius: 18,
      padding: '22px 28px',
      textAlign:'center',
      boxShadow: `0 0 ${24 + 18*blink}px ${VPAL.warm}66, 0 8px 22px rgba(31,58,58,0.12)`,
      border: `3px solid ${VPAL.ink}`,
      fontFamily: VSANS,
    }}>
      <div style={{
        fontFamily: VMONO, fontSize: 18, color: '#fff',
      ...
      <div style={{
        fontSize: 38, color:'#fff', fontWeight: 800, letterSpacing:'-0.01em',
        lineHeight: 1.15,
        opacity: blink,
      }}>
